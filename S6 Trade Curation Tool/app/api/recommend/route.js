...
Return ONLY valid JSON (no markdown, no explanation) with exactly these fields:
{
  ...
  "clientName": "the name of the client/business/property (e.g. 'The Roosevelt Hotel', 'Sunset Villas', 'John Smith') — look for fields like 'Client Name', 'Property Name', 'Business Name', 'Company', 'Name', or the first named entity in the brief",
  "location": "city and state or country (e.g. 'Nashville, TN', 'Miami, FL') — look for fields like 'Location', 'City', 'Address', or any geographic reference",
...
function parseBriefText(text) {
  ...
  const clientName = get('Company Name') || get('Client Name') || get('Client') || get('Property Name') || get('Business Name') || get('Company') || '';
  ...
  return { projectName, clientName, location, projectType, styleTags, paletteTags, avoidTags, galleryWall, pieceCount, rooms, keyThemes: styleTags, searchKeywords };
}

function defaultBrief() {
  return { projectName: '', clientName: '', location: '', projectType: 'other', styleTags: [], paletteTags: [], avoidTags: [], galleryWall: false, pieceCount: 10, rooms: [], keyThemes: [], searchKeywords: [] };
}
...
function scoreRecord(r, brief) {
  ...
  // that directly match catalog text — much more precise than style buckets
